...
<div class="container mt-5 mb-5">
    <div class="card">
        <div class="card-header">
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                ...
                <a href="<?= base_url('Export/gigi') ?>" class="btn btn-success">
  ...
  <td><?= 'Riwayat Penyakit Sekarang: ' . $rm['riwayat_penyakit_sekarang'] . ', ' .
           ...
           'Gigi Anomaly: ' . $rm['gigi_anomaly']; ?></td>
<td><?= 'Lab: ' . $rm['lab'] . ', ' .
 'Radiology: ' . $rm['radiology']; ?></td>
        ...
        $('#filter-dropdown').on('change', function () {
            ...
            if (filterValue) {
                table.columns().every(function () {
                    ...
                    if (filterType === 'no_rekam_medis') {
                        if (columnIndex === 2) { // No Rekam Medis
                            column.search(filterValue).draw();
                        } else {
                            column.search('').draw();
                        }
                    } else if (filterType === 'tanggal_kunjungan') {
                        if (columnIndex === 4) { // Tanggal Periksa
                            column.search(filterValue).draw();
                        } else {
                            column.search('').draw();
                        }
                ...
                table.columns().search('').draw(); // Clear filter
            }
        });
    });
</script>
