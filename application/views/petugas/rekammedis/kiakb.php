            ...
            <table class="table table-striped table-bordered" id="table-tbrekammediskb">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= 'Dilayani: ' . $rm['tgl_dilayani'].'Pesan Kembali: ' . $rm['tgl_pesan_kembali'].'Dicabut: ' . $rm['tgl_cabut']; ?></td>
                        <td><?= $rm['nama']; ?></td>
                        <td>
						<a href="<?= base_url('petugas/detail_data_rekammedis/') . $rm['id_rekam']; ?>" class="badge badge-warning">Detail</a>
                                                  </td>
...
$(document).ready(function() {
    ...
    $('#filter-dropdown-kb').on('change', function () {
        ...
        if (filterType === 'no_rekam_medis') {
            table.columns(1).search(selectedValue).draw(); // Index kolom mungkin perlu disesuaikan
        } else if (filterType === 'tanggal_kunjungan') {
            table.columns(3).search(selectedValue).draw(); // Index kolom mungkin perlu disesuaikan
        }
    });
});
</script>
