...
class anak_model extends CI_Model
    ...
    {
        $this->db->from($this->table);
        $this->db->where('id_anak', $this->session->userdata('id_anak'));
        $query = $this->db->get();
    ...
    public function getById($id) 
    {
        ...
        $query = $this->db->get();
        $this->db->get_where($this->table, ["id_anak" => $id])->row();
        return $query->row_array();
    }

    public function update($id_rekam_medis, $data) {
        $this->db->where('id_rekam_medis', $id_rekam_medis);
        $this->db->update('tbanak', $data);
    }

    public function insert($data) {
        $this->db->insert('tbanak', $data);
