...
class bumil_model extends CI_Model
    {
        public $table = 'tbbumil';
	public $id = 'tbbumil.id_bumil';
    ...
    {
        $this->db->from($this->table);
        $this->db->where('id_bumil',$id);
        $query = $this->db->get();
        $this->db->get_where($this->table, ["id_bumil" => $id])->row();
        return $query->row_array();
    }

    public function update($id_rekam_medis, $data) {
        $this->db->where('id_rekam_medis', $id_rekam_medis);
        $this->db->update('tbbumil', $data);
    }

  public function insert($data) {
    $this->db->insert('tbbumil', $data);
